...
const CodingBlocksModule = memo(({ addPoints }) => {
  ...
  return (
    <div className="w-full h-full relative overflow-hidden select-none flex flex-col">
      ...
      <div className="absolute top-4 left-1/2 -translate-x-1/2 z-30 flex items-center gap-4 glass-dark px-6 py-2.5 rounded-2xl border border-white/10 shadow-xl">
        ...
        <span className="text-[9px] font-black uppercase tracking-[0.3em] text-white/60">Programación Gestual</span>
        <div className="w-px h-4 bg-white/20" />
        <span className="text-[9px] font-black uppercase tracking-widest text-emerald-400">Nivel {levelIdx + 1}/{LEVELS.length} — {level.title}</span>
        <div className="w-px h-4 bg-white/20" />
      ...
      {/* Main layout */}
      <div className="flex-1 flex items-center justify-center gap-8 pt-16 pb-2 px-6">
        ...
        <div className="flex flex-col gap-3 w-52 flex-shrink-0">
          ...
          <div className="glass-dark rounded-2xl border border-white/10 p-3 min-h-[120px]">
            <p className="text-[8px] font-black uppercase tracking-widest text-white/40 mb-2">Secuencia ({sequence.length}/12)</p>
            <div className="flex flex-wrap gap-1.5">
              ...
              {!sequence.length && <p className="text-[9px] text-white/25 italic">Sin comandos</p>}
            </div>
          </div>

          {/* Repeat indicator */}
          <div className="glass-dark rounded-xl border border-emerald-500/20 px-3 py-2 flex items-center gap-2">
            <span className="text-[9px] font-black text-emerald-400 uppercase tracking-wider">🔁 Repetir:</span>
            <span className="text-[13px] font-black text-white">{repeatCount}×</span>
          </div>

          {/* Run/Reset */}
          <HandButton onClick={runSequence} dwellMs={700} variant="emerald" className="px-4 py-3 text-[11px]" >
            <Play size={13} /> Ejecutar
          </HandButton>
          <HandButton onClick={removeLastCommand} dwellMs={600} variant="orange" className="px-4 py-2.5 text-[10px]">
            <Trash2 size={12} /> Borrar último
          </HandButton>
          <HandButton onClick={resetLevel} dwellMs={700} variant="red" className="px-4 py-2.5 text-[10px]">
            <RotateCcw size={12} /> Reiniciar
          </HandButton>
          <HandButton onClick={() => setShowHint(true)} dwellMs={800} variant="default" className="px-4 py-2.5 text-[10px] !bg-white/5 !border-white/10">
            <Lightbulb size={12} /> Pista
          </HandButton>
          {success && (
            <HandButton onClick={nextLevel} dwellMs={700} variant="cyan" className="px-4 py-3 text-[11px]">
      ...
      <div className="relative z-20 pb-4 px-6">
        <p className="text-[8px] font-black uppercase tracking-[0.3em] text-white/30 text-center mb-2">Toca un bloque para añadirlo a la secuencia</p>
        <div className="flex items-center justify-center gap-2 flex-wrap">
          {COMMANDS.map(cmd => (
            <HandButton key={cmd.id} onClick={() => addCommand(cmd)} dwellMs={650} cooldownMs={400}
              variant={cmd.color} className="px-4 py-3 text-[10px] rounded-xl min-w-[90px]">
        ...
        {(success || fail) && (
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
            ...
            <div className={`px-6 py-3 rounded-2xl border shadow-xl text-center ${success ? 'border-emerald-500/50 bg-emerald-900/40' : 'border-red-500/50 bg-red-900/40'}`}>
              <p className="text-sm font-black uppercase tracking-widest">{success ? '⭐ ¡Llegaste a la meta!' : '❌ El robot se bloqueó'}</p>
              {!success && <p className="text-[10px] text-white/50 mt-1">Modifica la secuencia e intenta de nuevo</p>}
            </div>
          </motion.div>
        )}
      </AnimatePresence>
        ...
        {showHint && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }}
              ...
              <h3 className="text-lg font-display font-black italic uppercase text-yellow-400">Pista</h3>
        ...
        {showIntro && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }}
              ...
              <h2 className="text-3xl font-display font-black italic uppercase tracking-tight text-gradient">Programación Gestual</h2>
              <p className="text-[12px] text-white/60 leading-relaxed">
                Activa los <span className="text-emerald-400 font-black">bloques de comandos</span> con tu mano para construir una secuencia.
                Luego pulsa <span className="text-cyan-400 font-black">Ejecutar</span> para que el robot 🤖 siga tus instrucciones y llegue a la ⭐ estrella.
              </p>
        ...
        {allDone && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }}
              ...
              <div className="text-7xl">🏆</div>
              <h2 className="text-3xl font-display font-black italic uppercase text-emerald-400">¡Programador Experto!</h2>
              <p className="text-[13px] text-white/60 leading-relaxed">Completaste todos los niveles. Ya entiendes secuencias, condiciones y bucles — ¡los pilares de la programación!</p>
